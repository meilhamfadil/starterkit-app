    ...
    when (e) {
        is HttpException -> {
            ...
            try {
                ...
                baseDao =
                    Gson().fromJson<BaseResponse<T?>>(body!!.string(), BaseResponse::class.java)
                msg = baseDao?.message ?: ""
            } catch (exception: java.lang.Exception) {
                return when (exception) {
                    is UnknownHostException -> ApiResult(
                        false,
                        message = "Telah terjadi kesalahan ketika koneksi ke server: ${e.message}",
                        data = null
                    )
                    is SocketTimeoutException -> ApiResult(
                        false,
                        message = "Telah terjadi kesalahan ketika koneksi ke server: ${e.message}",
                        data = null
                    )
        ...
        is UnknownHostException -> return ApiResult(
            false,
            "Telah terjadi kesalahan ketika koneksi ke server: ${e.message}"
        )
        is SocketTimeoutException -> return ApiResult(
            false,
            "Telah terjadi kesalahan ketika koneksi ke server: ${e.message}"
        )
        is ConnectException -> return ApiResult(
            false,
            "Telah terjadi kesalahan ketika koneksi ke server: ${e.message}"
        )
        else -> return ApiResult(false, "Unknown error occured")
    }
}